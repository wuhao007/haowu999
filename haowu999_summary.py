...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        # 2. 统计特征 (用于 Sharpe 4.0 与阈值修正)
        rets = df['Close'].pct_change().dropna().tail(252*2)
        vol = rets.std() * np.sqrt(252)
        kurt = rets.kurtosis() # 峰度修正
        alpha = round(float((latest_p / df['Close'].tail(500).mean() - 1) * 100), 1)
        
        # 3. 动态修正阈值 (Volatility-Adjusted)
        v_adj = 1.0 + (vol - 0.3) * 0.5 # 以 30% 波动率为基准偏离
        bottom_target = round(0.45 / v_adj, 2)

        return {
            'name': name, 'ticker': ticker, 'ahr999': round(float(ahr), 3),
            'r2': round(float(r2), 4), 'alpha': alpha, 'vol': round(float(vol), 3),
            'kurt': round(float(kurt), 2), 'bottom_target': bottom_target,
            'price': round(latest_p, 2), 'p_buy': solve_target_price(bottom_target, ma200_sum_199, fit_p),
            'cur': 'HKD' if '.HK' in ticker else 'CNY' if '.SS' in ticker else 'USD',
            'is_pro': asset_cfg['is_pro'],
            'labels': df.tail(30)['Date'].dt.strftime('%m-%d').tolist(),
            'values': df.tail(30)['Close'].tolist(),
            'signal': "💎BOTTOM" if ahr < bottom_target else "✅INVEST" if ahr < 1.2 else "☕️WAIT"
        }
    except: return None
...
for i, item in enumerate(all_results):
    ...
    <div id='card_{i}' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="text-info small fw-bold">动态底部: {item['bottom_target']}</span>
        </div>
        <div class='{blur}'>
            <div style="height:60px; opacity:0.6;"><canvas id="c_{i}"></canvas></div>
            <div class="row g-2 text-center mt-3">
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">自适应抄底价</div><div class="fw-bold text-success val-ink" data-v='${item['p_buy']}'>${item['p_buy']}</div></div></div>
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">尾部峰度 (K)</div><div class="fw-bold text-danger">{item['kurt']}</div></div></div>
            </div>
...
<head>
    ...
    <title>Alpha HUB Zenith V238</title>
    ...
    <style>
        ...
        .val-blur::after { content: 'Perlin Layer Active'; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); color:rgba(255,255,255,0.1); font-size:0.4rem; font-weight:900; z-index:10; }
        .eye-btn { position:absolute; top:60px; right:20px; font-size:1.2rem; cursor:pointer; opacity:0.6; }
        #perlin-canvas { position:fixed; top:0; left:0; width:100%; height:100%; pointer-events:none; z-index:2000; opacity:0; }
...
<body>
    ...
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>波动率自适应政权 / Regime 4.0</span><span class="text-info">Institutional</span></div>
                <div id="v-regime" class="fs-4 fw-bold text-success">环境感知：波动率自适应激活</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：底部阈值已随日内波频动态修正 | REPLACE_TIME</p>
            </div>
        </div>
        ...
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="d-flex justify-content-between mb-3">
                    <div><div class="text-secondary small">Sharpe 4.0 (峰度修正)</div><div id="v-sharpe" class="fs-4 fw-bold text-info">--</div></div>
                    <div class="text-end"><div class="text-secondary small">主权分</div><div class="fs-4 fw-bold text-success">SSS</div></div>
                </div>
                <div class="text-secondary small">账户实时总净值 (Perlin 像素混淆)</div>
                <div id="v-total" class="fs-1 fw-bold text-info val-blur">$0.00</div>
                <p class="x-small text-muted mt-2">提示：Shadow Mode 32.0 开启。像素重构防御 AI OCR 逆向还原。</p>
            </div>
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-item" onclick="alert('Alpha Pro v238 | Perlin 加密引擎已激活')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function applyShadow() {
            ...
            // Perlin 像素重构交互
            document.getElementById('perlin-canvas').style.opacity = isShadow ? '0.1' : '0';
            if(isShadow && !perlinInterval) perlinInterval = setInterval(renderPerlin, 100);
            else if(!isShadow && perlinInterval) { clearInterval(perlinInterval); perlinInterval = null; }
        }
        function renderPerlin() {
            ...
            for (let i = 0; i < buffer.length; i++) {
                if(Math.random() > 0.98) buffer[i] = 0xFF0084FF; # 注入品牌色噪点
            }
            ctx.putImageData(idata, 0, 0);
        }
        function calcVault() {
            let total = 0; let weightedSharpe = 0; const h = {}; 
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                let alpha = parseFloat(i.dataset.alpha); let vol = parseFloat(i.dataset.vol); let kurt = parseFloat(i.dataset.kurt);
                h[i.dataset.ticker] = i.value;
                let usd = v * p * (c==='HKD'?0.128:c==='CNY'?0.138:1);
                total += usd;
                // Sharpe 4.0: (Alpha - 2%) / (Vol * Kurt)
                let s4 = (alpha - 2) / (vol * 100 * (1 + Math.abs(kurt)/10));
                weightedSharpe += (usd * s4);
            });
            ...
            if(total > 0) {
                document.getElementById('v-sharpe').innerText = (weightedSharpe / total).toFixed(2);
            }
        }
