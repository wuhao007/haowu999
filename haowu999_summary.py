...
def get_fx_rates():
    ...
    except: return {'HKD': 7.8, 'CNY': 7.2, 'USD': 1.0}
...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        return {
            ...
            'r2': round(float(r2), 4), 'p_buy': p_buy, 'p_sell': p_sell,
    ...
    <div class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        ...
        <div class='""" + blur_class + """'>
            ...
            <div class="row g-2 text-center mt-3">
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">抄底目标价</div><div class="fw-bold text-success data-p" data-usd='"""+str(item['p_buy'])+"""' data-cur='"""+item['cur']+"""'>$""" + str(item['p_buy']) + """</div></div></div>
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">止盈目标价</div><div class="fw-bold text-warning data-p" data-usd='"""+str(item['p_sell'])+"""' data-cur='"""+item['cur']+"""'>$""" + str(item['p_sell']) + """</div></div></div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2 mt-2 border-top border-secondary">
                <div class="text-secondary small">5年愿景: """ + str(item['growth_5y']) + """x</div>
...
<head>
    ...
    <title>Alpha Hub Pro V115</title>
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                <p style="color:#8e8e93; font-size:0.7rem; margin:0;">财富全周期决策中枢 | REPLACE_TIME</p>
                <select class="form-select-sm bg-dark border-secondary text-white" style="font-size:0.6rem; border-radius:10px;" onchange="changeDisplayCur(this.value)">
    ...
    <div id="tab-guide" class="tab-view container py-5 mt-4">
        <h2 style="font-weight:800;">量化百科</h2>
        <div class="accordion accordion-dark" id="g-acc">
            <div class="card bg-dark border-secondary mb-2 rounded-4">
                <div class="p-3 fw-bold text-primary" onclick="this.nextElementSibling.classList.toggle('d-none')">什么是 AHR999 / AHR999x ?</div>
                <div class="p-3 pt-0 small text-secondary d-none">
                    AHR999 是“抄底指标”，越低越便宜（建议 < 0.45 抄底）。<br>
                    AHR999x 是“逃顶指标”，越低越疯狂（建议 < 0.45 清仓）。
                </div>
            </div>
            <div class="card bg-dark border-secondary mb-2 rounded-4">
                <div class="p-3 fw-bold text-primary" onclick="this.nextElementSibling.classList.toggle('d-none')">R² 和 MAPE 代表什么 ?</div>
                <div class="p-3 pt-0 small text-secondary d-none">
                    <b>R² (信度)</b>：越高代表该资产越符合历史规律（0.95 以上极佳）。<br>
                    <b>MAPE (误差)</b>：越低代表当前价格预测越精准。
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="tab-settings" class="tab-view container py-5 mt-4">
        ...
        <div class="card bg-dark border-secondary p-3 rounded-4 mb-4">
            <div class="fw-bold text-primary mb-2">激活 Pro 权限</div>
        ...
        <div class="text-center text-secondary small mt-5">Alpha Hub Pro V115 | 全币种折算系统已激活</div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>信号</div>
        <div class="nav-item" onclick="switchTab('vault', this)">💰<br>资产</div>
        <div class="nav-item" onclick="switchTab('guide', this)">📖<br>指南</div>
    ...
    <script>
        ...
        function changeDisplayCur(cur) {
            document.querySelectorAll('.data-p').forEach(el => {
                let usd = parseFloat(el.dataset.usd);
                let assetCur = el.dataset.cur;
                // 先转回 USD 再转目标币种 (此处简化: 脚本里传的就是该资产本币)
                // 演示版: 假设全部按 USD 比例折算显示
                let displayVal = usd * (cur === 'HKD' ? FX.HKD : cur === 'CNY' ? FX.CNY : 1);
                el.innerText = (cur === 'CNY' ? '¥' : cur === 'HKD' ? 'HK$' : '$') + displayVal.toLocaleString();
            });
        }
        ...
        function calcVault() {
            ...
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                let usd = v * p;
                if(c === 'HKD') usd /= FX.HKD; if(c === 'CNY') usd /= FX.CNY;
                total += usd;
            });
        ...
        function renderChart(id, labels, data) {
            new Chart(document.getElementById(id), { type:'line', data:{ labels:labels, datasets:[{data:data, borderColor:'#0a84ff', borderWidth:2, pointRadius:0, fill:false}] }, options:{ responsive:true, maintainAspectRatio:false, scales:{x:{display:false},y:{display:false}}, plugins:{legend:{display:false}} } });
        }
