...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        # 策略灵敏度模拟数据 (用于前端 Playground)
        slope = model.coef_[0]
        annual_growth = (10**(slope * 365 / latest['Days']) - 1) * 100 if latest['Days'] > 0 else 0

        return {
            'name': name, 'ticker': ticker, 'ahr999': round(float(ahr), 3),
            'r2': round(float(r2), 4), 'growth': round(float(annual_growth), 1),
    ...
    <div class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        ...
        <div class='""" + blur + """'>
            ...
            <div class="row g-2 text-center mt-3">
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">年化自然增长</div><div class="fw-bold text-info">+""" + str(item['growth']) + """%</div></div></div>
...
<head>
    ...
    <title>Alpha Hub Pro V122</title>
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div style="background:#111; border-radius:10px; height:50px; border:1px dashed #333; margin-top:15px; display:flex; align-items:center; justify-content:center; color:#444; font-size:0.6rem;">AdMob Banner Loading...</div>
            <p class="x-small text-muted mt-3">财富全周期审计终端 | REPLACE_TIME</p>
        </div>
        <div class="px-3 mt-3">REPLACE_CARDS</div>
    </div>

    <div id="tab-vault" class="tab-view container py-5 mt-4 text-center">
        <h2 style="font-weight:800;">财富实验室</h2>
        <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4">
            <div class="text-secondary small">自由倒计时 (目标 $100万)</div>
            <div id="v-countdown" class="fs-1 fw-bold text-info">等待录入...</div>
            <div class="small text-success mt-2">账户总价值: <span id="v-total">$0.00</span></div>
        </div>
        <div class="card bg-dark border-secondary p-3 rounded-4 text-start">REPLACE_VAULT</div>
        <div class="mt-4"><button class="btn btn-outline-info btn-sm rounded-pill" onclick="alert('Alpha Sync: 持仓已加密同步至本地缓存')">一键多端同步财富口令</button></div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>信号</div>
        <div class="nav-item" onclick="switchTab('vault', this)">💰<br>自由</div>
        <div class="nav-item" onclick="alert('Alpha Pro v122 | 商业版全模块就绪')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function calcVault() {
            ...
            // 倒计时逻辑: 假设年化 15%, 计算到达 $100万所需年份
            if(total > 0) {
                let years = Math.log(1000000 / total) / Math.log(1 + 0.15);
                document.getElementById('v-countdown').innerText = years > 0 ? years.toFixed(1) + ' Years' : 'Freedom!';
            }
        }
