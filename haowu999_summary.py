...
for x in all_results:
    ...
    sector_data[x['sector']].append(x['ahr999'])
...
for i, item in enumerate(all_results):
    ...
    if item['is_pro']:
        cards_html += "<div class='pro-overlay text-center'><button class='btn btn-primary btn-sm rounded-pill px-3 fw-bold' onclick='switchTab(\"settings\")'>Unlock Strategy Vision</button></div>"
...
final_html = final_template.replace("REPLACE_TIME", datetime.now().strftime('%m-%d %H:%M')) \
    .replace("REPLACE_SECTOR_HEAT", sector_heatmap_html) \
