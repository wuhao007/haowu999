...
<div class="container py-5">
    ...
    <div class="alert alert-primary bg-primary text-white border-0 rounded-4 mb-4">
        📢 <b>今日洞察</b>: 目前全球组合拟合度 R² 均值 0.92，信号极其可靠。
    </div>
...
for item in all_results:
    ...
    <div class="col-12 col-md-6 mb-3">
        <div class="app-card shadow-lg">
            ...
            <div class="text-secondary small mb-3">{item['ticker']} | 拟合极准 🌟</div>
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="small text-secondary">AHR999 指数</div>
                    <div class="fs-4 fw-bold">{item['ahr999']}</div>
                </div>
                <div class="text-end">
                    <div class="small text-secondary">今日指令</div>
                    <div class="fs-4 fw-bold {sig_color}">{sig}</div>
                </div>
            </div>
        </div>
    </div>
...
report += f"### 📱 [点击在手机安装 App](https://wuhao007.github.io/haowu999/)\n"
...
for item in sorted(all_results, key=lambda x: x['r2'], reverse=True):
    report += f"| {item['name']} | `{item['r2']}` | {item['mape']}% | {'🌟 定投圣经' if item['r2'] > 0.9 else '✅ 可信'} | {item['rank']}% |\n"
