...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        cap_eff = round(alpha / (vol * 100 + 0.1), 2) # Alpha per unit of Risk
        
        # 因子分类逻辑
        if vol > 0.6: factor = "Aggressive"
        elif 'BTC' in ticker or 'ETH' in ticker: factor = "Growth"
        elif 'GC=F' in ticker or 'SI=F' in ticker: factor = "Defensive"
        else: factor = "Value"

        return {
            'name': name, 'ticker': ticker, 'sector': sector, 'factor': factor,
            'ahr999': round(float(ahr), 3), 'r2': round(float(r2), 4), 'alpha': alpha,
            'vol': round(float(vol), 3), 'cap_eff': cap_eff,
...
all_results.sort(key=lambda x: x['cap_eff'], reverse=True) # 按资本效率排序
...
for i, item in enumerate(all_results):
    ...
    f_color = "#ff453a" if item['factor'] == "Aggressive" else "#0a84ff" if item['factor'] == "Growth" else "#32d74b"
    ...
    <div id='card_{i}' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span style='color:{f_color}; font-size:0.75rem; font-weight:900;'>因子: {item['factor']}</span>
        </div>
        <div class='{blur}'>
            ...
            <div class="row g-2 text-center mt-3">
                ...
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">资本效率</div><div class="fw-bold text-info">{item['cap_eff']}</div></div></div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-3 mt-2 border-top border-secondary border-opacity-25">
                <div class="text-secondary small">AHR: {item['ahr999']} | Alpha: +{item['alpha']}%</div>
                <div class="fs-5 fw-bold text-primary">{item['signal']}</div>
            </div>
        </div>
    """
    if item['is_pro']:
        cards_html += "<div class='pro-overlay text-center'><button class='btn btn-primary btn-sm rounded-pill px-3 fw-bold' onclick='switchTab(\"settings\")'>Unlock Efficiency Pro</button></div>"
    cards_html += "</div>"
    
    scripts_html += f"renderChart('c_{i}', {json.dumps(item['labels'])}, {json.dumps(item['values'])});\n"
    vault_rows += f"<div class='mb-3 d-flex justify-content-between align-items-center'><div class='small text-secondary title-ink' data-orig='{item['name']}'>{item['name']} ({item['cur']})</div><input type='number' class='hold-in h-curtain' data-ticker='{item['ticker']}' data-price='{item['price']}' data-cur='{item['cur']}' data-eff='{item['cap_eff']}' data-factor='{item['factor']}' placeholder='Units' onchange='calcVault()' style='width:80px; background:#111; border:1px solid #333; color:#fff; border-radius:6px; text-align:center;'></div>"
...
<head>
    ...
    <title>Alpha HUB Elite V214</title>
    ...
    <style>
        ...
        .h-curtain { filter: brightness(0); transition: 0.3s; cursor: pointer; }
        .h-curtain:hover, .h-curtain:active { filter: brightness(1); }
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>组合资本效率 / Capital Efficiency</span><span class="text-info">Elite</span></div>
                <div id="v-eff" class="fs-4 fw-bold text-success">等待数据录入...</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：基于 Alpha/Vol 风险资本产出比 | REPLACE_TIME</p>
            </div>
        </div>
    ...
    <div id="tab-vault" class="tab-view container py-5 mt-4 text-center">
        <h2 style="font-weight:800;">因子归因矩阵</h2>
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="text-secondary small mb-2">持仓因子构成 (Factor Exposure)</div>
                <div id="factor-grid" class="d-flex flex-wrap gap-2 mb-3"></div>
                <div class="mt-3 pt-3 border-top border-secondary border-opacity-25">
                    <div class="text-secondary small">账户总价值 (触控可见)</div>
                    <div id="v-total" class="fs-1 fw-bold text-info h-curtain">$0.00</div>
                </div>
            </div>
            <div class="card bg-dark border-secondary p-3 rounded-4 text-start">REPLACE_VAULT</div>
        </div>
        <div class="mt-4"><button class="btn btn-outline-info btn-sm rounded-pill w-100" onclick="alert('主权密钥已同步至本地')">🔐 导出量子级加密密钥 6.0</button></div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>机会</div>
        <div class="nav-item" onclick="switchTab('vault', this)">💰<br>主权</div>
        <div class="nav-item" onclick="alert('Alpha Pro v214 | 资本效率引擎已激活')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function applyShadow() {
            ...
            document.querySelectorAll('.h-curtain').forEach(el => {
                if(isShadow) el.classList.add('h-curtain'); else el.classList.remove('h-curtain');
            });
        ...
        function calcVault() {
            let total = 0; let weightedEff = 0; const h = {}; const factors = {};
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                let eff = parseFloat(i.dataset.eff); let f = i.dataset.factor;
                h[i.dataset.ticker] = i.value;
                let usd = v * p * (c==='HKD'?0.128:c==='CNY'?0.138:1);
                total += usd;
                weightedEff += (usd * eff);
                if(v > 0) factors[f] = (factors[f] || 0) + usd;
            });
            ...
            if(total > 0) {
                document.getElementById('v-eff').innerText = `大师级 (${(weightedEff / total).toFixed(2)})`;
                let fHtml = "";
                for(let k in factors) {
                    let pct = (factors[k]/total*100).toFixed(0);
                    fHtml += `<span class="badge border border-secondary text-secondary" style="font-size:0.6rem">${k}: ${pct}%</span>`;
                }
                document.getElementById('factor-grid').innerHTML = fHtml;
            }
        }
