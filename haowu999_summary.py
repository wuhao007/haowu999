...
# 3. 市场广度审计 (Market Breadth)
breadth_count = len([x for x in all_results if x['ahr999'] < 1.0])
breadth_score = int(breadth_count / len(all_results) * 100)
    ...
    <div id='card_{i}' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        ...
        <div class='{blur}'>
            ...
            <div class="row g-2 text-center mt-3">
                ...
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">凯利 7.0</div><div class="fw-bold text-warning">{round(65 * (1 - fx['VIX']/100), 1)}%</div></div></div>
            </div>
...
<head>
    ...
    <title>Alpha HUB Singularity V251</title>
    ...
    <style>
        ...
        .jitter-active { animation: jitter 0.05s infinite; }
        @keyframes jitter {
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>市场‘广度收敛’审计 / Breadth</span><span class="text-info">REPLACE_B_SCORE%</span></div>
                <div id="v-msg" class="fs-4 fw-bold text-success">系统评价: 广泛收敛 (大底确认)</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：基于全市场 AHR 信号同步性审计 | REPLACE_TIME</p>
            </div>
        </div>
        ...
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="d-flex justify-content-between mb-3">
                    <div><div class="text-secondary small">VIX 恐慌修正凯利 7.0</div><div id="v-kelly" class="fs-4 fw-bold text-info">x0.82</div></div>
                    <div class="text-end"><div class="text-secondary small">主权分</div><div class="fs-4 fw-bold text-success">Elite</div></div>
                </div>
                <div class="text-secondary small">账户实时总净值 (量子波振保护)</div>
                <div id="v-total" class="fs-1 fw-bold text-info val-blur">$0.00</div>
                <p class="x-small text-muted mt-2">提示：Shadow Mode 45.0 开启。金额处于高频量子波振中，防御 OCR 深度特征抓取。</p>
            </div>
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-item" onclick="alert('Alpha Pro v251 | 广度审计引擎已并网')">⚙️<br>设置</div>
    </nav>
        ...
        function applyShadow() {
            ...
            document.querySelectorAll('.val-blur').forEach(el => {
                if(isShadow) {
                    ...
                    el.classList.add('jitter-active');
                } else {
...
final_html = final_template.replace("REPLACE_TIME", datetime.now().strftime('%m-%d %H:%M')) \
    .replace("REPLACE_B_SCORE", str(breadth_score)) \
