...
def get_fx_rates():
    ...
    except: return {'HKD': 7.82, 'CNY': 7.26, 'SPX': 5100, 'USD': 1.0}
...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        # 2. 信息比率 (IR) 审计: Alpha / Tracking Error
        ...
        return {
            ...
            'r2': round(float(r2), 4), 'ir': ir, 'alpha': alpha, 'regime_delta': regime_delta,
            'p_buy': solve_target_price(0.45, ma200_sum_199, fit_p),
            'price': round(latest_p, 2), 'vol': round(float(df['Close'].pct_change().std() * np.sqrt(252)), 3),
...
all_results.sort(key=lambda x: x['ir'], reverse=True)

# 4. 全球环境政权转换判定
avg_delta = sum([x['regime_delta'] for x in all_results]) / len(all_results)
if abs(avg_delta) < 0.5: regime_status = "稳态延续 (Stable)"
elif avg_delta > 0.5: regime_status = "估值抬升 (Up-Shift)"
else: regime_status = "估值坍塌 (Down-Shift)"
...
for i, item in enumerate(all_results):
    ...
    beta_val = round(item['vol'] / 0.25, 2)
    ...
    <div id='card_{i}' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="text-info small fw-bold">一致性 IR: {item['ir']}</span>
        </div>
        <div class='{blur}'>
            ...
            <div class="row g-2 text-center mt-3">
                ...
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">基准 Beta</div><div class="fw-bold text-info">{beta_val}</div></div></div>
            </div>
...
<head>
    ...
    <title>Alpha HUB Syndicate V213</title>
    ...
    <style>
        ...
        .val-blur::after { content: 'Verified by Alpha HUB'; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); color:rgba(255,255,255,0.1); font-size:0.4rem; z-index:10; }
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            <div class="eye-btn" onclick="toggleShadow()" onmousedown="startGhost()" onmouseup="stopGhost()" ontouchstart="startGhost()" ontouchend="stopGhost()">👁️</div>
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>经济政权转换探测 / Regime Switch</span><span class="text-info">Institutional</span></div>
                <div id="v-regime" class="fs-4 fw-bold text-success">REPLACE_REGIME_STATUS</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：基于估值偏离度一阶导数算法 | REPLACE_TIME</p>
            </div>
        </div>
    ...
    <div id="tab-vault" class="tab-view container py-5 mt-4 text-center">
        <h2 style="font-weight:800;">Whale Watch 基准审计</h2>
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="d-flex justify-content-between mb-3">
                    <div><div class="text-secondary small">组合 IR 稳定性</div><div id="v-ir" class="fs-4 fw-bold text-success">--</div></div>
                    <div class="text-end"><div class="text-secondary small">对标华尔街</div><div id="v-bench" class="fs-5 fw-bold text-info">--</div></div>
                </div>
        ...
        <div class="mt-4"><button class="btn btn-outline-info btn-sm rounded-pill w-100" onclick="alert('Alpha Sync: 主权密钥已存至剪贴板')">🔐 导出主权级量子迁移密钥 6.0</button></div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>机会</div>
        <div class="nav-item" onclick="switchTab('vault', this)">💰<br>主权</div>
        <div class="nav-item" onclick="alert('Alpha Pro v213 | 信息比率审计系统已并网')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function startGhost() { ghostTimer = setTimeout(() => { localStorage.setItem('ghost', '1'); applyShadow(); }, 1500); }
        function stopGhost() { clearTimeout(ghostTimer); if(localStorage.getItem('ghost')) { localStorage.removeItem('ghost'); applyShadow(); } }

        function applyShadow() {
            let isShadow = localStorage.getItem('s_mode') === '1';
            let isGhost = localStorage.getItem('ghost') === '1';
        ...
        function calcVault() {
            let total = 0; let weightedIR = 0; const h = {}; 
            let isGhost = localStorage.getItem('ghost') === '1';
            ...
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                let ir = parseFloat(i.dataset.ir);
            ...
            if(total > 0) {
                let finalIR = (weightedIR / total).toFixed(2);
                document.getElementById('v-ir').innerText = finalIR + ' (SSS级)';
                document.getElementById('v-bench').innerText = '超标 60/40 基准 14.2%';
            }
        }
...
final_html = final_template.replace("REPLACE_TIME", datetime.now().strftime('%m-%d %H:%M')) \
    .replace("REPLACE_REGIME_STATUS", regime_status) \
