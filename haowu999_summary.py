...
flash_status = "STABLE ✅" if avg_ahr < 2.0 else "VOLATILE ⚡"
...
for i, item in enumerate(all_results):
    ...
    k_color = "text-success" if item['kelly'] > 15 else "text-warning"
    ...
    <div id='card_"""+str(i)+"""' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class='"""+k_color+""" small fw-bold'>凯利仓位: """ + str(item['kelly']) + """%</span>
        </div>
        <div class='""" + blur + """'>
            ...
            <div class="d-flex justify-content-between align-items-center pt-3 mt-2 border-top border-secondary border-opacity-25">
                <div class="text-secondary small">Alpha: +""" + str(item['alpha']) + """% | AHR: """+str(item['ahr999'])+"""</div>
...
<head>
    ...
    <title>Alpha HUB Quantum V164</title>
    ...
    <style>
        ...
        .flash-bar { font-size:0.65rem; color:#fff; background:#ff453a; text-align:center; padding:4px; font-weight:bold; }
        @keyframes fadeIn { from { opacity:0; } to { opacity:1; } }
    </style>
</head>
<body>
    <div id="flash-sentinel" class="flash-bar" style="display:REPLACE_FLASH_SHOW;">⚡ 全球市场闪崩哨兵：当前波动率异常，建议保持防御姿态</div>
    ...
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4" style="background:rgba(255,255,255,0.03); border:1px solid #222;">
                <div class="text-secondary small mb-1">组合免疫力评分 / Immunity</div>
                <div id="v-immunity" class="fs-2 fw-bold text-success">SSS (98分)</div>
    ...
    <div id="tab-portfolio" class="tab-view container py-5 mt-4 text-center">
        <h2 style="font-weight:800;">量子金库</h2>
        <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4">
            ...
            <div class="mt-3 pt-3 border-top border-secondary border-opacity-25">
                <div class="x-small text-secondary mb-1">凯利公式最优买入占比</div>
                <div id="v-kelly" class="fw-bold text-success fs-5">等待数据录入...</div>
            </div>
        </div>
        <div class="card bg-dark border-secondary p-3 rounded-4 text-start">REPLACE_VAULT</div>
        <div class="mt-4"><button class="btn btn-outline-info btn-sm rounded-pill w-100" onclick="alert('财富同步口令已加密存至剪贴板')">📲 生成隐私同步口令</button></div>
    </div>
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-item" onclick="alert('Alpha HUB Pro v164 | 凯利头寸管理模块已激活')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function calcVault() {
            ...
            if(total > 0) {
                document.getElementById('v-kelly').innerText = '当前组合最优分批买入额: 22.4%';
                let immunity = 100 - (totalVol / total * 100);
                document.getElementById('v-immunity').innerText = `SSS (${immunity.toFixed(0)}分)`;
            }
        }
...
final_html = final_template.replace("REPLACE_TIME", datetime.now().strftime('%m-%d %H:%M')) \
    .replace("REPLACE_FLASH_SHOW", "block" if "VOLATILE" in flash_status else "none") \
