...
all_results.sort(key=lambda x: x['alpha'], reverse=True) # 按战绩排行
    ...
    <div class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        ...
        <div class='""" + blur + """'>
            ...
            <div class="row g-2 text-center mt-3">
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.55rem">夏普比率 (Sharpe)</div><div class="fw-bold text-info">""" + str(item['sharpe']) + """</div></div></div>
...
<head>
    ...
    <title>Alpha Hub Pro V125</title>
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header">
            ...
            <div class="mt-3 p-3 rounded-4" style="background:rgba(255,255,255,0.03); border:1px solid #222;">
                <div class="x-small text-secondary mb-2">🏆 实战超额收益战绩榜 (过去2年)</div>
                REPLACE_LEADERBOARD
                <p class="x-small text-muted mt-2 mb-0">更新于: REPLACE_TIME</p>
            </div>
        </div>
    ...
    <div id="tab-portfolio" class="tab-view container py-5 mt-4 text-center">
        ...
        <div class="alert alert-dark border-secondary x-small text-secondary">
            录入持仓后，Alpha Hub 会自动对比 <b>Sharpe Ratio</b> 为您提供调仓建议。
        </div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>战绩信号</div>
        <div class="nav-item" onclick="switchTab('portfolio', this)">💰<br>本地金库</div>
        <div class="nav-item" onclick="alert('Alpha Pro v125 | 机构级审计系统已激活')">⚙️<br>设置</div>
    </nav>
