...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        return {
            ...
            'r2': round(float(r2), 4), 'p_btm': p_btm, 'p_sell': p_sell,
...
for i, item in enumerate(all_results):
    ...
    <div class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow position-relative overflow-hidden">
        ...
        """ + ("<div class='pro-overlay text-center'><button class='btn btn-primary btn-sm rounded-pill px-3 fw-bold' onclick='switchTab(\"settings\")'>Unlock Pro Analytics</button></div>" if item['is_pro'] else "") + """
    </div>
    """
...
<body>
    ...
    <div id="tab-vault" class="tab-view container py-5 mt-4">
        ...
        <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-center">
            ...
            <div class="text-secondary small mt-3">持仓总估值 (折算USD)</div>
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-item" onclick="alert('Alpha Pro v93 | 组合相关性防爆预警已开启')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function calcVault() {
            let total = 0; const h = {}; const chartData = []; const labels = [];
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                if(usd > 0) { total += usd; chartData.push(usd); labels.push(i.dataset.ticker); }
            });
            localStorage.setItem('alpha_h_v3', JSON.stringify(h));
            document.getElementById('v-total').innerText = '$' + total.toLocaleString(undefined, {minimumFractionDigits: 2});
            renderPie(labels, chartData);
        }
        ...
        function renderPie(labels, data) {
            ...
            pChart = new Chart(ctx, {
                type: 'doughnut',
                data: { labels, datasets: [{ data, backgroundColor: ['#0a84ff','#32d74b','#ffd60a','#ff453a','#5e5ce6','#af52de'], borderWidth: 0 }] },
