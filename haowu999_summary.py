...
# 3. 宏观政权与马尔可夫崩盘概率 (模拟种子)
avg_ahr = sum([x['ahr999'] for x in all_results]) / len(all_results)
crash_prob = int(min(95, (avg_ahr**2) * 5)) # 泡沫越大概率越高
...
<head>
    ...
    <title>Alpha HUB Singularity V240</title>
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>马尔可夫‘大波动’概率 / Markov Risk</span><span class="text-info">Institutional</span></div>
                <div id="v-crash" class="fs-4 fw-bold text-success">未来30天回撤概率: REPLACE_CRASH%</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：基于平均 AHR 指数加速比审计 | REPLACE_TIME</p>
            </div>
        </div>
    ...
    <div id="tab-vault" class="tab-view container py-5 mt-4 text-center">
        ...
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="d-flex justify-content-between mb-3">
                    <div><div class="text-secondary small">组合多样化指数 (Entropy)</div><div id="v-entropy" class="fs-4 fw-bold text-success">--</div></div>
                    <div class="text-end"><div class="text-secondary small">主权分</div><div class="fs-4 fw-bold text-info">Elite</div></div>
                </div>
                <div class="text-secondary small">账户实时总净值 (全量环境保护)</div>
        ...
        <div class="mt-4"><button class="btn btn-outline-info btn-sm rounded-pill w-100" onclick="exportHeritage()">🔐 导出主权级‘全环境’冷密钥 7.0</button></div>
    </div>

    <nav class="nav-bar">
        <div class="nav-item active" onclick="switchTab('home', this)">📊<br>机会</div>
        <div class="nav-item" onclick="switchTab('vault', this)">💰<br>主权</div>
        <div class="nav-item" onclick="alert('Alpha Pro v240 | 组合熵审计系统已并网')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function exportHeritage() {
            let data = { h: localStorage.getItem('alpha_h_v4'), j: localStorage.getItem('trade_journal'), s: localStorage.getItem('s_mode'), p: '1' };
            prompt('您的全加密主权密钥 7.0 (离线遗产)：', btoa(json.stringify(data)));
        }
...
final_html = final_template.replace("REPLACE_TIME", datetime.now().strftime('%m-%d %H:%M')) \
    .replace("REPLACE_CRASH", str(crash_prob)) \
