...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        return {
            ...
            'r2': round(float(model.score(x, y)), 4), 'price': round(float(latest['Close']), 2),
...
# 计算资产相关性
corr_matrix = pd.DataFrame(price_matrix).pct_change().corr().round(2).to_dict()
...
for i, item in enumerate(all_results):
    ...
    <div class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg overflow-hidden position-relative">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="text-success small fw-bold">拟合精度 """ + str(int(item['r2']*100)) + """%</span>
        </div>
        <div class='""" + blur + """'>
            ...
            <div class="row g-2 text-center mt-3">
                ...
                <div class="col-6"><div class="p-2 rounded bg-black border border-secondary"><div class="small text-secondary" style="font-size:0.6rem">今日动作</div><div class="fw-bold text-primary">""" + signal + """</div></div></div>
            </div>
            <div class="mt-3 pt-2 border-top border-secondary text-secondary small">AHR999: """ + str(item['ahr999']) + """ | 建议: 下跌至挂单价分批买入</div>
        </div>
        """ + ("<div class='pro-overlay text-center'><button class='btn btn-primary btn-sm rounded-pill fw-bold' onclick='switchTab(\"settings\")'>Unlock Backtest</button></div>" if item['is_pro'] else "") + """
    </div>
    """
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header">
            ...
            <p style="color:#8e8e93; font-size:0.8rem;">策略净值实证终端 | REPLACE_TIME</p>
        </div>
        <div class="px-3">REPLACE_CARDS</div>
    </div>

    <div id="tab-risk" class="tab-view container py-5 mt-4">
        <h2 style="font-weight:800;">相关性热力图</h2>
        <div class="alert alert-info bg-dark border-info border-opacity-25 text-info small mt-3" style="font-size:0.6rem;">
            数值 1.0 代表涨跌完全同步。建议配置 <b>负相关或低相关</b> 资产以对冲风险。
        </div>
    ...
    <div id="tab-portfolio" class="tab-view container py-5 mt-4 text-center">
        ...
        <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4">
            <div class="text-secondary small">总资产估值 (USD)</div>
            <div id="v-total" class="fs-1 fw-bold text-info">$0.00</div>
            <div class="small text-success mt-2">实时汇率接口已激活</div>
        </div>
