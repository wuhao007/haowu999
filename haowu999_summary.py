...
def analyze_asset(asset_cfg, base_start='2010-01-01'):
    ...
    try:
        ...
        avg_loss = abs(rets[rets < 0].mean())
        expectancy = round((win_rate * avg_win - (1-win_rate) * avg_loss) * 1000, 2) # 每千刀期望回报

        return {
            'name': name, 'ticker': ticker, 'ahr999': round(float(ahr), 3),
            'r2': round(float(r2), 4), 'expectancy': expectancy,
...
all_results.sort(key=lambda x: x['expectancy'], reverse=True) # 按博弈期望排序
...
for i, item in enumerate(all_results):
    ...
    <div id='card_{i}' class="card bg-dark border-secondary rounded-4 p-3 mb-3 shadow-lg position-relative overflow-hidden">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="text-info small fw-bold">博弈期望: {item['expectancy']}</span>
        </div>
        <div class='{blur}'>
...
<head>
    ...
    <title>Alpha HUB Apex V239</title>
    ...
    <style>
        ...
        .val-blur::after { content: 'Logic Redirection Active'; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); color:rgba(255,255,255,0.1); font-size:0.4rem; font-weight:900; z-index:10; }
        .eye-btn { position:absolute; top:60px; right:20px; font-size:1.2rem; cursor:pointer; opacity:0.6; }
        .greek-var { font-family: "Times New Roman", serif; font-style: italic; color: #0a84ff !important; font-weight: bold; }
...
<body>
    <div id="tab-home" class="tab-view active-tab">
        <div class="header text-center">
            ...
            <div class="mt-3 p-3 rounded-4 shadow-sm" style="background:#111; border:1px solid #333;">
                <div class="d-flex justify-content-between x-small text-secondary mb-1"><span>今日全市场‘博弈期望’ / Expectancy</span><span class="text-info">Institutional</span></div>
                <div id="v-exp" class="fs-4 fw-bold text-success">等待资产录入...</div>
                <p class="x-small text-muted mt-2 mb-0">系统分析：基于 Alpha/MAPE 复利胜率拟合 | REPLACE_TIME</p>
            </div>
        </div>
        ...
        <div id="audit-report">
            <div class="card bg-dark border-primary p-4 rounded-4 shadow mb-4 text-start">
                <div class="d-flex justify-content-between mb-3">
                    <div><div class="text-secondary small">组合凯利 4.0 动态修正</div><div id="v-kelly" class="fs-4 fw-bold text-info">x0.92</div></div>
                    <div class="text-end"><div class="text-secondary small">主权分</div><div class="fs-4 fw-bold text-success">Elite</div></div>
                </div>
                <div class="text-secondary small">账户实时总净值 (变量映射保护)</div>
                <div id="v-total" class="fs-1 fw-bold text-info val-blur">$0.00</div>
                <p class="x-small text-muted mt-2">提示：Shadow Mode 33.0 开启。所有金额已被映射为逻辑变量。</p>
            </div>
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-item" onclick="alert('Alpha Pro v239 | 变量映射系统已激活')">⚙️<br>设置</div>
    </nav>
    ...
    <script>
        ...
        function calcVault() {
            let total = 0; let totalExp = 0; const h = {}; 
            let isShadow = localStorage.getItem('s_mode') === '1';
            ...
            document.querySelectorAll('.hold-in').forEach(i => {
                ...
                let exp = parseFloat(i.dataset.exp);
            ...
            if(isShadow) {
                // 变量映射逻辑: Shadow Mode 33.0
                const vars = ["Σ_α", "φ_β", "Ω_γ", "Ψ_δ", "Ξ_ε"];
                if(total < 1000) displayVal = "χ_min";
                else if(total < 10000) displayVal = vars[1];
                else if(total < 100000) displayVal = vars[0];
                else displayVal = "Λ_Max";
                document.getElementById('v-total').classList.add('greek-var');
            } else {
                document.getElementById('v-total').classList.remove('greek-var');
            }
            document.getElementById('v-total').innerText = displayVal;
            
            if(total > 0) {
                document.getElementById('v-exp').innerText = `每千刀期望: $${(totalExp/total).toFixed(2)}`;
            }
        }
