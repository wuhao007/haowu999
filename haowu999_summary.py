...
for i, item in enumerate(results):
    pro = '<span style="background:#0a84ff; font-size:0.6rem; padding:2px 6px; border-radius:4px;">PRO</span>' if item['is_pro'] else ''
    cards_html += f"""
    <div style="background:#1c1c1e; border-radius:20px; padding:20px; margin-bottom:15px; border:1px solid #333;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            <span style="font-weight:700; font-size:1.1rem;">{item['name']} {pro}</span>
            <span style="color:#32d74b; font-size:0.7rem;">拟合信度 {item['r2']}</span>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="text-align:right;"><div style="color:#8e8e93; font-size:0.6rem;">动作</div><div style="font-size:1.1rem; font-weight:800; color:#0a84ff;">{'抄底' if item['ahr999']<0.45 else '定投' if item['ahr999']<1.2 else '观望'}</div></div>
        </div>
    </div>
    """
...
<head>
    ...
    <style>body { background:#000; color:#fff; font-family:-apple-system; padding:20px; padding-bottom:90px; }</style>
</head>
<body>
    <h2 style="font-weight:800;">投资 <span style="color:#0a84ff;">PRO</span></h2>
    <p style="color:#8e8e93; font-size:0.7rem;">对数回归实证终端 | REPLACE_TIME</p>
